...
class App extends Component<PropsFromRedux> {
  verifyAuth = () => {
    ...
    if (token) {
      try {
        const decoded = jwt.verify(token, process.env.REACT_APP_SECRET_CODE + '') as JwtPayload;
...
const mapDispatch = {
  loginSuccess: (username: string) => ({ type: ACTIONS.AUTH_LOGIN_SUCCESS, payload: username }),
