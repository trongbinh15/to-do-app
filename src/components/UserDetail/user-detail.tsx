			...
			<>
				...
				{isNew ? (
					...
					<>
						...
						<button className="btn add-task-btn" onClick={() => this.addTask()}>
							Add task
						</button>
					</>
				)}
...
const mapState = (
	state: RootState,
	ownProps: RouteComponentProps<ParamRoute>,
) => {
	const user = state.users.find(
		(x: IUser) => x.id === ownProps.match.params.id,
	);
	const tasks = state.tasks.filter(
		(x: ITask) => x.userId === ownProps.match.params.id,
	);
